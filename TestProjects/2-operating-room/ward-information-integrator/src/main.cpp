// main.cpp -- device app -- Ward Information Integrator (OR). Constructs each
...
// pull messages: common.harpia types + case_validation_cache); wire the
// generated CRUDL DAO / REST bindings for those the way HarpiaTest/app_example/consumer does.
//
...
// OR devices' own outbound streams.
//
...
int main() {
    ...
    const std::string ward_id = "OR-1";

    {
        ::ward_telemetry_batch m;
        m.set_ward_id(ward_id);
        m.set_device_id("ANES-3300");
        m.set_metric_name("sevoflurane_percent");
        m.set_metric_value(2.1f);
    ...
    }
    {
        ::ward_alarm_relay m;
        m.set_ward_id(ward_id);
        m.set_device_id("HLM-4100");
        m.set_alarm_code(233);
        m.set_active(0);
        dump("ward_alarm_relay", m);
    }
    {
        ::ward_audit_relay m;
        m.set_source_id("ROBOT-5001");
        m.set_category("calibration");
        m.set_detail("arm 2 calibration pass");
    ...
    std::cout << "Ward Information Integrator (OR) device app: done" << std::endl;
    return 0;
}
