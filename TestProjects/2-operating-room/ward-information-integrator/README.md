...
| [`ward_information_integrator.harpia`](./ward_information_integrator.harpia) | uplink messages to the HMS (`ward_telemetry_batch`, `ward_alarm_relay`, `ward_audit_relay`, `store_forward_replay`, `uplink_status`) as `push`; cached downlink data (`case_validation_cache` + the `common.harpia` types) as `pull` |
