// main.cpp -- device app -- constructs each outbound message the Sequential Compression Device (SCD) publishes
...
int main() {
    ...
    for (int i = 0; i < iterations; ++i) {
        {
            ::compression_cycle m;
            m.set_sleeve_pressure_mmhg(static_cast<float>(jitter(40, 5)));
            m.set_cycle_index(static_cast<int>(std::llround(jitter(0, 0))));
            m.set_chamber("ankle");
            std::string js;
            (void) ::google::protobuf::util::MessageToJsonString(m, &js, jopts);
            std::cout << "compression_cycle: " << js << std::endl;
        }
        {
            ::scd_runtime_log m;
            m.set_total_runtime_minutes(static_cast<int>(std::llround(jitter(120, 0))));
            std::string js;
            (void) ::google::protobuf::util::MessageToJsonString(m, &js, jopts);
            std::cout << "scd_runtime_log: " << js << std::endl;
        }
        {
            ::sleeve_disconnected_alert m;
            m.set_leg_side("left");
            m.set_disconnected(static_cast<int>(std::llround(jitter(0, 0))));
            std::string js;
            (void) ::google::protobuf::util::MessageToJsonString(m, &js, jopts);
            std::cout << "sleeve_disconnected_alert: " << js << std::endl;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(200));
    }
    std::cout << "Sequential Compression Device (SCD) device app: done" << std::endl;
    return 0;
}
