...
- On first generation the pipeline drops a `schema_registry/` sidecar next to each `.harpia` (stable field numbers). It is **git-ignored** in `TestProjects/` (`.gitignore`: `TestProjects/**/schema_registry/`) -- these are throwaway examples, regenerate it locally; do not hand-edit. In a real project it is committed source.
