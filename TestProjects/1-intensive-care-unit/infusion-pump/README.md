...
tree** (`TestProjects/_gen/infusion-pump/`, gitignored) so the build container -- which only mounts the
...
bash ./run_harpia.sh TestProjects/1-intensive-care-unit/infusion-pump TestProjects/_gen/infusion-pump --no-build
...
bash docker/run.sh bash -c '
  cmake -S TestProjects/1-intensive-care-unit/infusion-pump -B TestProjects/1-intensive-care-unit/infusion-pump/_build -DHARPIA_GEN=/harpia/TestProjects/_gen/infusion-pump &&
  cmake --build TestProjects/1-intensive-care-unit/infusion-pump/_build -j"$(nproc)"'

# 3. run the executables (inside the image)
bash docker/run.sh bash -c './TestProjects/1-intensive-care-unit/infusion-pump/_build/device_app
  ./TestProjects/1-intensive-care-unit/infusion-pump/_build/human_mock | head'
```
...
Clean up with `rm -rf TestProjects/_gen/infusion-pump TestProjects/1-intensive-care-unit/infusion-pump/_build`.
