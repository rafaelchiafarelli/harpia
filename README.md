...
| 14 | generated-code unit tests | 🟡 per-message C++ unit tests as an opt-in CTest target (`-DHARPIA_BUILD_TESTS=ON`): simple field access + CRUDL round-trip (14.1/14.2), SOAP access-rights credential gate (14.3), access-modifier constraint enforcement (14.4, PRIMARY KEY uniqueness), JSON parser round-trip + checker (14.5), XML parser round-trip (14.6), and live HTTP-API tests — REST JSON CRUD (14.7/14.10) and SOAP-over-HTTP with credential (14.8/14.9) against real in-process servers; REST XML content-negotiation and app-level (all-good/crash/slower/non-parseable) tests deferred |
