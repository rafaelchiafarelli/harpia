...
- Non-additive schema **type changes** — `migrate_<name>` handles rename and
  drop (see Stage 8 above) but not a column's type changing between versions.
  SQLite has no `ALTER COLUMN TYPE`, so this needs the create/copy/drop/rename
  table-rebuild dance, a materially different strategy from Postgres's direct
  `ALTER COLUMN TYPE`.
