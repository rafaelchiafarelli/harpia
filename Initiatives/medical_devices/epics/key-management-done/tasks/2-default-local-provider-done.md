## Default local `KeyProvider` + fail-safe acknowledgment gate

Landed in `8b2953b`.

- **Depends on:** task 1 merged.
- **Deliverable:** a concrete default `KeyProvider` (e.g. platform-
  keystore/TPM-sealed local storage) implementing task 1's interface, for
...
- **Guarantees:** the default impl passes task 1's own wrap/unwrap/rotate
  test suite unmodified (interface conformance); the acknowledgment gate
  blocks silent use under a PHI-at-scale profile.
- **Out of scope:** KMS/HSM reference adapter (task 5), crypto-shredding
  (task 3), zeroization/audit wiring (task 4).
- **Tests:**
  - Unit: default impl satisfies task 1's interface contract tests.
...
- `Crypto/runtime/harpia_key_provider_local.h` — `LocalKeyProvider :
  ...
  task 1's `InMemoryKeyProvider`). Ctor throws `LocalKeyProviderRefused` when
  `phi_at_scale && !acknowledged`. `local_key_provider_acknowledged()`
  reads `HARPIA_ACK_LOCAL_KEY_PROVIDER` (`1`/`true`/`yes`, any case) as a
  convenience source for the `acknowledged` field. Cipher is still task 1's
...
- **Deferred:** the compliance profile that sets `phi_at_scale` is wired
  from `ComplianceContext` at generation time by **the db-encryption epic** (first
  consumer to copy these headers into generated output); task 2 supplies the
