...
| [events-callbacks](events-callbacks/) | `event[cached/not-cached]`, detached-thread callback dispatch | **done** (task 1 → `Callback/` module + `EventChannel<T>` + CRUDL `publish()` on create/update; task 2 → detached-thread dispatch + callback exception isolation; task 3 → OnChange `AuditSink` for `phi` (`phi_event_dispatch` / `phi_event_onchange`) + headline round-trip; `ComplianceReport/` note in `process-artifacts/tasks/events-callbacks-phi-audit-note.md`) |
