...
| [dds-transport](dds-transport/) | DDS transport adapter (ASTM F2761 / OpenICE-class bedside bus) | **in progress** (task 1 `dds-grammar-support` + 2a `dds-vendor-and-spike` done — `dds` modifier + `Message.is_dds`; Eclipse Cyclone DDS 0.10.5 + `ddscxx` vendored (`third_party/cyclonedds{,-cxx}/`), built in the Docker image, spike proves pub/sub + QoS + security hook. 2b, 3, 4, 5 open. `deadline[ms]` QoS carved out pending domain expert) |
