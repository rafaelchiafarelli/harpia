...
| [zmq-lifecycle](zmq-lifecycle/) | ZMQ CURVE security (shipped, verify only) + full `stream` lifecycle | in progress (task 1 `stream-control` **done** — `<name>_stream` setup/read/stop + timeout watchdog on `stream` messages; task 2 `data-connection-reclamation` scoped, not started; task 3 `authentication-layer` blocked on transport-authn; task 4 `windows-build-verification` env-blocked → Windows session-line) |
