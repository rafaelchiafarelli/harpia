...
| [dds-transport](dds-transport/) | DDS transport adapter (ASTM F2761 / OpenICE-class bedside bus) | **in progress** (task 1 `dds-grammar-support` done — `dds` modifier + `Message.is_dds`, flag only; 2a `dds-vendor-and-spike`, 2b, 3, 4, 5 open. DDS vendor is 2a's spike-then-commit, starting candidate Eclipse Cyclone DDS; `deadline[ms]` QoS carved out pending domain expert) |
