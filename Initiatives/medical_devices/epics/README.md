...
| [sdc-biceps](sdc-biceps/) | IEEE 11073 SDC / BICEPS bindings (scoping + WS-Discovery responder only) | **in progress** (planning done — tasks 1 `ws-discovery-test-harness`, 2 `ws-discovery-responder`, 3 `metric-alert-context-design-doc` scoped. Task 1 **done**: `UnitTests/wsdiscovery_harness.py`, stdlib-only WS-DD 2009 probe/resolve client for task 2's integration test. Task 3 **done**: `sdc-biceps/sdc_biceps_design.md` — Metric/Alert/Context mapping analysis, `event ≈ Metric` / `critical event ≈ Alert` / `Context ≈ no-grammar-yet` hypothesis, 7 gaps + V1–V7 validation list, no grammar change. Task 2 (responder) remains. Planning decisions: responder advertises a fixed generic DPWS type + Harpia-namespaced scope URI, no new `.harpia` modifier this pass; `Database/SoapAdapter.py` / `WsdlAdapter.py` read-only) |
