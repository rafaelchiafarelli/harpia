...
| [static-fuzz-ci](static-fuzz-ci-done/) | static / fuzz analysis CI | **done** (5 tasks: 1, 2, 3, 4a, 4b). `cppcheck --enable=warning,portability` pytest gate (`test_cppcheck.py`) over the generated `cpp/` tree, empty baseline (`cppcheck_suppressions.txt`), no CERT addon (removed upstream). One hand-rolled ASan/UBSan fuzz driver (`UnitTests/fuzz/harpia_fuzz_main.cpp`, in-process `FuzzMsg` descriptor, seeded xorshift mutator, `__asan_set_death_callback` replay dump) covering the JSON (`serialize::detail::from_json`), XML (`xml::from_xml`) and SOAP (`soap::message_from_request`) parser entry points via `test_fuzz_parsers.py` (`@pytest.mark.fuzz`, `pytest.ini`; runs by default, bounded to seconds). Task 4 split → **4a** extracted the SOAP parse seam (`SoapAdapter/runtime/harpia_soap.h`: `parse_envelope`/`find_operation`/`message_from_request`; `soap.h.tmpl` rewired behavior-preserving; 12 `golden/soap/*` re-blessed; new `SoapAdapter/` module dir) since the parse path was welded into the crow handler with no standalone entry point — the epic's one generator-source + golden change — + **4b** the SOAP fuzz target on it. Also fixed `Sha256::buf_` uninit (`Compliance/runtime/harpia_session.h`, surfaced by the task-1 gate) + its golden. Docker: 508 passed, 4 skipped. |
