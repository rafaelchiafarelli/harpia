...
| [test-progress/](test-progress/test-progress.md) | **Scoping doc only, not started.** Live per-test progress (`% done`, pass/fail tally, elapsed) for `Docker/run.sh pytest` and the raw `docker run` suite invocations — today `-q` buffers and the `[ NN%]` marker lags a whole line. Recommends an env-gated `UnitTests/conftest.py` hook (no new dependency) writing a JSONL progress file + a `Docker/suite-status.sh` reader; ~2 tasks. |
