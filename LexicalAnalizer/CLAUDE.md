...
- `PROTECTED r'protected '` / `OPEN r'open '` (message-level-hardening
  initiative, protected-open-modifiers epic, task 1) are the *RBAC/session-
  gate* axis, independent of `CRITICAL`'s criticality axis and `DDS`'s
  transport-selection axis. Same keyword-only, trailing-space shape as
  `CRITICAL`/`DDS`, same slot (before `message `). Consumed by
  `Message/Message.py`: each lands in `message.access_modifiers` and sets
  `message.is_protected` / `message.is_open` respectively. Neither present ->
  the project-wide hardening default applies unchanged (byte-identical output
  to before this token existed). Both present on one message is a hard
  generation-time `Types.CONFLICTING_HARDENING_MODIFIERS` error, never a
  silent precedence rule. Flag only in this task: no adapter reads either flag
  yet (REST/SOAP/gRPC wiring is epic task 3). Composes freely with any
