...
- `templates/adapter.h.tmpl` — the C++ header text (Python `str.format` placeholders). Provides `to_json(msg, *out)`, `from_json(in, *msg)`, `is_valid_json(in)`. `from_json`/`is_valid_json` pass `JsonParseOptions{{ignore_unknown_fields = true}}` to `JsonStringToMessage` (plain single-arg `JsonStringToMessage` defaults that to `false`) so a JSON payload carrying a key this schema doesn't recognize (a newer peer's added field) still parses, matching proto3 binary/XML's own tolerant-unknown-field behavior — added as part of the message-versioning effort's parse-boundary hardening (shipped 2026-08-22).

## Key facts / gotchas
- **Unified `toString` (the serialization epic):** `harpia::serialize::to_string(msg, Format::JSON)` (`SerializeAdapter/`) is the single cross-format entry point going forward; for JSON it calls the exact same `MessageToJsonString`/`JsonStringToMessage` (same options) this wrapper does, so JSON output is unchanged. This module's per-message `to_json`/`from_json`/`is_valid_json` stay as-is (golden-frozen); the façade sits above them.
