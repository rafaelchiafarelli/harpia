...
Motivated by an existing Android fleet — see
`../README.md` §4 for the selection history. This thread scopes Java as
...
fit a single sitting): [track-j-java-target.md](track-j-java-target.md).
...
| 13 ZMQ | cppzmq (native, linked) | **Real, but smaller than C++'s own Windows story turned out to be** | `org.zeromq:jeromq` is a pure-Java reimplementation of the ZMTP wire protocol — **no JNI, no native library, no per-platform build** at all. Socket-pattern API is close enough to cppzmq that the origin-id scheme ports as the portable algorithm it already is. **One unconfirmed fact**: JeroMQ claims CURVE support in recent releases, unverified against a pinned version — needs a real check before the CURVE sub-slice, same discipline this repo applied to the SOCI::PostgreSQL alias question before assuming an answer. |
...
Unchanged from `../README.md` §3: one C++ data point wasn't enough to
design a `DbBackend`-style seam, and it still isn't enough with Java as a
...
Same shape as `HARPIA_DB_BACKEND` → `get_backend()` and as `../README.md`
§5 describes generally: thread a `HARPIA_GEN_LANG` env var (default
`cpp`) through `main.py`, resolved once.

## 6. Slice order

See [track-j-java-target.md](track-j-java-target.md) — 27 sessions,
...
Same order of magnitude as `../README.md` §6's general estimate — a
multi-session epic, comparable to (or larger than) the Postgres-backend
effort. Two things push it slightly larger than a Python estimate would
...
4. Python is still next in line after Java — this thread's existence
   doesn't mean Python no longer matters, see `../README.md` §4.
