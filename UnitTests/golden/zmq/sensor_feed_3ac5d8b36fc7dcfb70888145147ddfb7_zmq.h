...
#ifndef HARPIA_ZMQ_SENSOR_FEED_3ac5d8b36fc7dcfb70888145147ddfb7
#define HARPIA_ZMQ_SENSOR_FEED_3ac5d8b36fc7dcfb70888145147ddfb7
...
// Guarded separately from the per-message HARPIA_ZMQ_SENSOR_FEED_3ac5d8b36fc7dcfb70888145147ddfb7 so these definitions stay
...
// pub/sub (streaming/event): sensor_feed_publisher publishes (stamping origin), sensor_feed_subscriber receives.
class sensor_feed_publisher {
public:
    sensor_feed_publisher(::zmq::context_t& ctx, const std::string& endpoint)
        : sensor_feed_publisher(ctx, endpoint, origin_id()) {}
    ...
    static const std::string& origin_id() {
        static const std::string id = "189656764142729907";
        return id;
    }
    const std::string& origin() const { return origin_; }
    bool publish(const ::sensor_feed& msg) {
        ::sensor_feed stamped = msg;
...
class sensor_feed_subscriber {
public:
    // Trailing curve keys default to disabled (empty), so callers who don't
    // pass any get today's plaintext behavior unchanged.
    sensor_feed_subscriber(::zmq::context_t& ctx, const std::string& endpoint,
...
// stream (process.md 13.2): sensor_feed_stream is the lifecycle consumer surface
// layered on sensor_feed_subscriber's SUB socket.
...
// `::sensor_feed`; StreamStatus / StreamConfig / stream_config_valid() are shared
// across the translation unit (see HARPIA_ZMQ_STREAM_DEFINED above).
struct sensor_feed_read_result {
    StreamStatus status;
    ::std::optional<::sensor_feed> msg;
};

class sensor_feed_stream {
public:
    ...
    // plaintext, exactly like sensor_feed_subscriber.
...
#endif  // HARPIA_ZMQ_SENSOR_FEED_3ac5d8b36fc7dcfb70888145147ddfb7
