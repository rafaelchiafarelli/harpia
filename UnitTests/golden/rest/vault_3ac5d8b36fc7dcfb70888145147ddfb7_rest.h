...
#ifndef HARPIA_REST_VAULT_3ac5d8b36fc7dcfb70888145147ddfb7
#define HARPIA_REST_VAULT_3ac5d8b36fc7dcfb70888145147ddfb7
...
// RESTful CRUD for vault, backed by the CRUDL DAO. Register on a
// crow::SimpleApp with a base path (e.g. "/project/v1"); routes are then:
//   GET    <base>/vault        list   (?limit=&offset= to paginate)
...
// vault declares a "pagination[size]" field (its size becomes the default
...
//               X-User: vault and X-Pswd: 3ac5d8b36fc7dcfb70888145147ddfb7, or it is rejected with 401
...
// Role gate for vault (transport-authn epic, tasks 4 + 5). Identity is either
...
inline bool authz_vault(const crow::request& req, crow::response& res,
    ...
    switch (::harpia::rbac::decide(cn, op, "vault")) {
...
inline bool wants_xml_vault(const crow::request& req) {
    return req.get_header_value("Accept").find("xml") != std::string::npos;
}
inline bool body_xml_vault(const crow::request& req) {
    return req.get_header_value("Content-Type").find("xml") != std::string::npos;
}
inline bool parse_vault(const crow::request& req, ::vault* msg) {
    return body_xml_vault(req) ? ::harpia::xml::from_xml(req.body, msg)
                                : ::harpia::json::from_json(req.body, msg);
}
inline void serialize_vault(const crow::request& req,
                             const ::vault& msg, crow::response& res) {
    if (wants_xml_vault(req)) {
...
inline void register_vault(crow::SimpleApp& app, ::soci::session& db,
                            const std::string& base) {
    const std::string col = base + "/vault";
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authz_vault(req, res, ::harpia::rbac::Operation::list)) { res.end(); return; }
            ::harpia::db::vault_dao dao(*dbp);
            std::vector<::vault> rows;
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res, int64_t id) {
            if (!authz_vault(req, res, ::harpia::rbac::Operation::read)) { res.end(); return; }
            ::harpia::db::vault_dao dao(*dbp);
            ::vault msg;
            if (!dao.read(id, &msg)) { res.code = 404; res.end(); return; }
            serialize_vault(req, msg, res);
            res.end();
        });

    app.route_dynamic(col).methods(crow::HTTPMethod::POST)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authz_vault(req, res, ::harpia::rbac::Operation::create)) { res.end(); return; }
            ::harpia::db::vault_dao dao(*dbp);
            ::vault msg;
            if (!parse_vault(req, &msg)) { res.code = 400; res.end(); return; }
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::PUT)(
        [dbp](const crow::request& req, crow::response& res, int64_t) {
            if (!authz_vault(req, res, ::harpia::rbac::Operation::update)) { res.end(); return; }
            ::harpia::db::vault_dao dao(*dbp);
            ::vault msg;
            if (!parse_vault(req, &msg)) { res.code = 400; res.end(); return; }
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::DELETE)(
        [dbp](const crow::request& req, crow::response& res, int64_t id) {
            if (!authz_vault(req, res, ::harpia::rbac::Operation::remove)) { res.end(); return; }
            ::harpia::db::vault_dao dao(*dbp);
...
#endif  // HARPIA_REST_VAULT_3ac5d8b36fc7dcfb70888145147ddfb7
