...
#ifndef HARPIA_REST_RECEPTION_DESK_3ac5d8b36fc7dcfb70888145147ddfb7
#define HARPIA_REST_RECEPTION_DESK_3ac5d8b36fc7dcfb70888145147ddfb7
...
// RESTful CRUD for reception_desk, backed by the CRUDL DAO. Register on a
// crow::SimpleApp with a base path (e.g. "/project/v1"); routes are then:
//   GET    <base>/reception_desk        list   (?limit=&offset= to paginate)
...
// reception_desk declares a "pagination[size]" field (its size becomes the default
...
//               X-User: reception_desk and X-Pswd: 3ac5d8b36fc7dcfb70888145147ddfb7, or it is rejected with 401
...
// True iff the request carries the correct credential for reception_desk. Exposed so it
// can be unit-tested directly.
inline bool authorized_reception_desk(const crow::request& req) {
    return req.get_header_value("X-User") == "reception_desk" &&
           req.get_header_value("X-Pswd") == "3ac5d8b36fc7dcfb70888145147ddfb7";
}

// content negotiation: XML when asked for, JSON otherwise
inline bool wants_xml_reception_desk(const crow::request& req) {
    return req.get_header_value("Accept").find("xml") != std::string::npos;
}
inline bool body_xml_reception_desk(const crow::request& req) {
    return req.get_header_value("Content-Type").find("xml") != std::string::npos;
}
inline bool parse_reception_desk(const crow::request& req, ::reception_desk* msg) {
    return body_xml_reception_desk(req) ? ::harpia::xml::from_xml(req.body, msg)
                                : ::harpia::json::from_json(req.body, msg);
}
inline void serialize_reception_desk(const crow::request& req,
                             const ::reception_desk& msg, crow::response& res) {
...
#endif  // HARPIA_REST_RECEPTION_DESK_3ac5d8b36fc7dcfb70888145147ddfb7
