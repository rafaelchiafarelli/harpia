...
// the request must carry X-User: alarm_event and X-Pswd: 3ac5d8b36fc7dcfb70888145147ddfb7, or it is rejected
// with HTTP 401 before the operation runs.
namespace harpia {
namespace rest {

// True iff the request carries the correct credential for alarm_event. Exposed so it
// can be unit-tested directly.
inline bool authorized_alarm_event(const crow::request& req) {
    return req.get_header_value("X-User") == "alarm_event" &&
...
inline void register_alarm_event(crow::SimpleApp& app, ::soci::session& db,
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authorized_alarm_event(req)) { res.code = 401; res.end(); return; }
