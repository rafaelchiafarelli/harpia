...
#ifndef HARPIA_REST_ALARM_EVENT_3ac5d8b36fc7dcfb70888145147ddfb7
#define HARPIA_REST_ALARM_EVENT_3ac5d8b36fc7dcfb70888145147ddfb7
...
// RESTful CRUD for alarm_event, backed by the CRUDL DAO. Register on a
// crow::SimpleApp with a base path (e.g. "/project/v1"); routes are then:
//   GET    <base>/alarm_event        list   (?limit=&offset= to paginate)
...
// alarm_event declares a "pagination[size]" field (its size becomes the default
...
//               X-User: alarm_event and X-Pswd: 3ac5d8b36fc7dcfb70888145147ddfb7, or it is rejected with 401
...
// Role gate for alarm_event (transport-authn epic, task 4). Resolves the verified
...
inline bool authz_alarm_event(const crow::request& req, crow::response& res,
                         ::harpia::rbac::Operation op) {
    switch (::harpia::rbac::decide(req.client_cert_cn, op, "alarm_event")) {
...
inline bool wants_xml_alarm_event(const crow::request& req) {
    return req.get_header_value("Accept").find("xml") != std::string::npos;
}
inline bool body_xml_alarm_event(const crow::request& req) {
    return req.get_header_value("Content-Type").find("xml") != std::string::npos;
}
inline bool parse_alarm_event(const crow::request& req, ::alarm_event* msg) {
    return body_xml_alarm_event(req) ? ::harpia::xml::from_xml(req.body, msg)
                                : ::harpia::json::from_json(req.body, msg);
}
inline void serialize_alarm_event(const crow::request& req,
                             const ::alarm_event& msg, crow::response& res) {
    if (wants_xml_alarm_event(req)) {
...
inline void register_alarm_event(crow::SimpleApp& app, ::soci::session& db,
                            const std::string& base) {
    const std::string col = base + "/alarm_event";
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authz_alarm_event(req, res, ::harpia::rbac::Operation::list)) { res.end(); return; }
            ::harpia::db::alarm_event_dao dao(*dbp);
            std::vector<::alarm_event> rows;
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res, int64_t id) {
            if (!authz_alarm_event(req, res, ::harpia::rbac::Operation::read)) { res.end(); return; }
            ::harpia::db::alarm_event_dao dao(*dbp);
            ::alarm_event msg;
            if (!dao.read(id, &msg)) { res.code = 404; res.end(); return; }
            serialize_alarm_event(req, msg, res);
            res.end();
        });

    app.route_dynamic(col).methods(crow::HTTPMethod::POST)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authz_alarm_event(req, res, ::harpia::rbac::Operation::create)) { res.end(); return; }
            ::harpia::db::alarm_event_dao dao(*dbp);
            ::alarm_event msg;
            if (!parse_alarm_event(req, &msg)) { res.code = 400; res.end(); return; }
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::PUT)(
        [dbp](const crow::request& req, crow::response& res, int64_t) {
            if (!authz_alarm_event(req, res, ::harpia::rbac::Operation::update)) { res.end(); return; }
            ::harpia::db::alarm_event_dao dao(*dbp);
            ::alarm_event msg;
            if (!parse_alarm_event(req, &msg)) { res.code = 400; res.end(); return; }
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::DELETE)(
        [dbp](const crow::request& req, crow::response& res, int64_t id) {
            if (!authz_alarm_event(req, res, ::harpia::rbac::Operation::remove)) { res.end(); return; }
            ::harpia::db::alarm_event_dao dao(*dbp);
...
#endif  // HARPIA_REST_ALARM_EVENT_3ac5d8b36fc7dcfb70888145147ddfb7
