...
// the request must carry X-User: telemetry and X-Pswd: 3ac5d8b36fc7dcfb70888145147ddfb7, or it is rejected
...
// True iff the request carries the correct credential for telemetry. Exposed so it
// can be unit-tested directly.
inline bool authorized_telemetry(const crow::request& req) {
    return req.get_header_value("X-User") == "telemetry" &&
...
inline void register_telemetry(crow::SimpleApp& app, ::soci::session& db,
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authorized_telemetry(req)) { res.code = 401; res.end(); return; }
