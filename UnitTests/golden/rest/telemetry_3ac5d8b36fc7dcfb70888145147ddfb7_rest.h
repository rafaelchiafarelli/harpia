...
#ifndef HARPIA_REST_TELEMETRY_3ac5d8b36fc7dcfb70888145147ddfb7
#define HARPIA_REST_TELEMETRY_3ac5d8b36fc7dcfb70888145147ddfb7
...
// RESTful CRUD for telemetry, backed by the CRUDL DAO. Register on a
// crow::SimpleApp with a base path (e.g. "/project/v1"); routes are then:
//   GET    <base>/telemetry        list   (?limit=&offset= to paginate)
...
// telemetry declares a "pagination[size]" field (its size becomes the default
...
//               X-User: telemetry and X-Pswd: 3ac5d8b36fc7dcfb70888145147ddfb7, or it is rejected with 401
...
// Role gate for telemetry (transport-authn epic, task 4). Resolves the verified
...
inline bool authz_telemetry(const crow::request& req, crow::response& res,
                         ::harpia::rbac::Operation op) {
    switch (::harpia::rbac::decide(req.client_cert_cn, op, "telemetry")) {
...
inline bool wants_xml_telemetry(const crow::request& req) {
    return req.get_header_value("Accept").find("xml") != std::string::npos;
}
inline bool body_xml_telemetry(const crow::request& req) {
    return req.get_header_value("Content-Type").find("xml") != std::string::npos;
}
inline bool parse_telemetry(const crow::request& req, ::telemetry* msg) {
    return body_xml_telemetry(req) ? ::harpia::xml::from_xml(req.body, msg)
                                : ::harpia::json::from_json(req.body, msg);
}
inline void serialize_telemetry(const crow::request& req,
                             const ::telemetry& msg, crow::response& res) {
    if (wants_xml_telemetry(req)) {
...
inline void register_telemetry(crow::SimpleApp& app, ::soci::session& db,
                            const std::string& base) {
    const std::string col = base + "/telemetry";
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authz_telemetry(req, res, ::harpia::rbac::Operation::list)) { res.end(); return; }
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res, int64_t id) {
            if (!authz_telemetry(req, res, ::harpia::rbac::Operation::read)) { res.end(); return; }
            ::harpia::db::telemetry_dao dao(*dbp);
            ::telemetry msg;
            if (!dao.read(id, &msg)) { res.code = 404; res.end(); return; }
            serialize_telemetry(req, msg, res);
            res.end();
        });

    app.route_dynamic(col).methods(crow::HTTPMethod::POST)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authz_telemetry(req, res, ::harpia::rbac::Operation::create)) { res.end(); return; }
            ::harpia::db::telemetry_dao dao(*dbp);
            ::telemetry msg;
            if (!parse_telemetry(req, &msg)) { res.code = 400; res.end(); return; }
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::PUT)(
        [dbp](const crow::request& req, crow::response& res, int64_t) {
            if (!authz_telemetry(req, res, ::harpia::rbac::Operation::update)) { res.end(); return; }
            ::harpia::db::telemetry_dao dao(*dbp);
            ::telemetry msg;
            if (!parse_telemetry(req, &msg)) { res.code = 400; res.end(); return; }
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::DELETE)(
        [dbp](const crow::request& req, crow::response& res, int64_t id) {
            if (!authz_telemetry(req, res, ::harpia::rbac::Operation::remove)) { res.end(); return; }
            ::harpia::db::telemetry_dao dao(*dbp);
...
#endif  // HARPIA_REST_TELEMETRY_3ac5d8b36fc7dcfb70888145147ddfb7
