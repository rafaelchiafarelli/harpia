...
#ifndef HARPIA_REST_BEACON_LOG_3ac5d8b36fc7dcfb70888145147ddfb7
#define HARPIA_REST_BEACON_LOG_3ac5d8b36fc7dcfb70888145147ddfb7
...
//               X-User: beacon_log and X-Pswd: 3ac5d8b36fc7dcfb70888145147ddfb7, or it is rejected with 401
...
// Role gate for beacon_log (transport-authn epic, task 4). Resolves the verified
...
inline bool authz_beacon_log(const crow::request& req, crow::response& res,
                         ::harpia::rbac::Operation op) {
    switch (::harpia::rbac::decide(req.client_cert_cn, op, "beacon_log")) {
...
inline void register_beacon_log(crow::SimpleApp& app, ::soci::session& db,
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authz_beacon_log(req, res, ::harpia::rbac::Operation::list)) { res.end(); return; }
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res, int64_t id) {
            if (!authz_beacon_log(req, res, ::harpia::rbac::Operation::read)) { res.end(); return; }
            ::harpia::db::beacon_log_dao dao(*dbp);
            ::beacon_log msg;
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::POST)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authz_beacon_log(req, res, ::harpia::rbac::Operation::create)) { res.end(); return; }
            ::harpia::db::beacon_log_dao dao(*dbp);
            ::beacon_log msg;
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::PUT)(
        [dbp](const crow::request& req, crow::response& res, int64_t) {
            if (!authz_beacon_log(req, res, ::harpia::rbac::Operation::update)) { res.end(); return; }
            ::harpia::db::beacon_log_dao dao(*dbp);
            ::beacon_log msg;
            if (!parse_beacon_log(req, &msg)) { res.code = 400; res.end(); return; }
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::DELETE)(
        [dbp](const crow::request& req, crow::response& res, int64_t id) {
            if (!authz_beacon_log(req, res, ::harpia::rbac::Operation::remove)) { res.end(); return; }
            ::harpia::db::beacon_log_dao dao(*dbp);
...
#endif  // HARPIA_REST_BEACON_LOG_3ac5d8b36fc7dcfb70888145147ddfb7
