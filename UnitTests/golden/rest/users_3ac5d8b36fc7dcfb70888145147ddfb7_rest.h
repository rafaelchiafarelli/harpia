...
#ifndef HARPIA_REST_USERS_3ac5d8b36fc7dcfb70888145147ddfb7
#define HARPIA_REST_USERS_3ac5d8b36fc7dcfb70888145147ddfb7
...
//               X-User: users and X-Pswd: 3ac5d8b36fc7dcfb70888145147ddfb7, or it is rejected with 401
...
// Role gate for users (transport-authn epic, task 4). Resolves the verified
...
inline bool authz_users(const crow::request& req, crow::response& res,
                         ::harpia::rbac::Operation op) {
    switch (::harpia::rbac::decide(req.client_cert_cn, op, "users")) {
...
inline void register_users(crow::SimpleApp& app, ::soci::session& db,
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authz_users(req, res, ::harpia::rbac::Operation::list)) { res.end(); return; }
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res, int64_t id) {
            if (!authz_users(req, res, ::harpia::rbac::Operation::read)) { res.end(); return; }
            ::harpia::db::users_dao dao(*dbp);
            ::users msg;
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::POST)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authz_users(req, res, ::harpia::rbac::Operation::create)) { res.end(); return; }
            ::harpia::db::users_dao dao(*dbp);
            ::users msg;
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::PUT)(
        [dbp](const crow::request& req, crow::response& res, int64_t) {
            if (!authz_users(req, res, ::harpia::rbac::Operation::update)) { res.end(); return; }
            ::harpia::db::users_dao dao(*dbp);
            ::users msg;
            if (!parse_users(req, &msg)) { res.code = 400; res.end(); return; }
    ...
    app.route_dynamic(item).methods(crow::HTTPMethod::DELETE)(
        [dbp](const crow::request& req, crow::response& res, int64_t id) {
            if (!authz_users(req, res, ::harpia::rbac::Operation::remove)) { res.end(); return; }
            ::harpia::db::users_dao dao(*dbp);
...
#endif  // HARPIA_REST_USERS_3ac5d8b36fc7dcfb70888145147ddfb7
