...
//
// Unit tests for the generated vault message, CRUDL DAO and SOAP access:
...
#include "db/vault_3ac5d8b36fc7dcfb70888145147ddfb7_crudl.h"
#include "soap/vault_3ac5d8b36fc7dcfb70888145147ddfb7_soap.h"
#include "json/vault_3ac5d8b36fc7dcfb70888145147ddfb7_json.h"
#include "xml/vault_3ac5d8b36fc7dcfb70888145147ddfb7_xml.h"
...
int simple_access() {
    ...
    m.set_secret_code("secret_code_a");
    ...
    if (m.secret_code() != "secret_code_a") return 11;
...
int database_roundtrip() {
    ...
    if (got.secret_code() != "secret_code_a") return 24;
...
int access_rights() {
    ...
    if (::harpia::rbac::decide("", Operation::create, "vault")
...
int access_modifiers() {
    ...
    dup.set_secret_code("secret_code_b");
...
int json_parser() {
    ...
    if (b.secret_code() != "secret_code_a") return 62;
...
int xml_parser() {
    ...
    if (b.secret_code() != "secret_code_a") return 72;
...
int rest_api() {
    ...
    harpia::rest::register_vault(app, db, "/api/v1");
    ...
    int code = 0;
    do {
        auto gl = cli.Get("/api/v1/vault");
        if (!gl || gl.status != 401) { code = 93; break; }
        auto gi = cli.Get("/api/v1/vault/1");
        if (!gi || gi.status != 401) { code = 94; break; }
        auto po = cli.Post("/api/v1/vault", "{}", "application/json");
        if (!po || po.status != 401) { code = 95; break; }
        auto de = cli.Delete("/api/v1/vault/1");
...
int soap_api() {
    ...
    crow::SimpleApp app;
    app.loglevel(crow::LogLevel::Warning);
    harpia::soap::register_vault_soap(app, db, "/soap");
    auto fut = app.bindaddr("127.0.0.1").port(0).multithreaded().run_async();
    app.wait_for_server_start();
    const int port = app.port();
    ...
    const std::string setEnv = "<soap:Envelope><soap:Body><set><vault></vault></set></soap:Body></soap:Envelope>";
    int code = 0;
    do {
        auto g = cli.Post("/soap/vault", getEnv, "text/xml");
        if (!g || g.status != 401) { code = 103; break; }
        auto s = cli.Post("/soap/vault", setEnv, "text/xml");
