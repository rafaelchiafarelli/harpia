...
int access_rights() {
    ...
    if (::harpia::rbac::decide("", Operation::create, "vip_users")
...
int rest_api() {
    ...
    int code = 0;
    do {
        auto gl = cli.Get("/api/v1/vip_users");
        if (!gl || gl.status != 401) { code = 93; break; }
        auto gi = cli.Get("/api/v1/vip_users/1");
        if (!gi || gi.status != 401) { code = 94; break; }
        auto po = cli.Post("/api/v1/vip_users", "{}", "application/json");
        if (!po || po.status != 401) { code = 95; break; }
        auto de = cli.Delete("/api/v1/vip_users/1");
...
int soap_api() {
    ...
    const std::string setEnv = "<soap:Envelope><soap:Body><set><vip_users></vip_users></set></soap:Body></soap:Envelope>";
    int code = 0;
    do {
