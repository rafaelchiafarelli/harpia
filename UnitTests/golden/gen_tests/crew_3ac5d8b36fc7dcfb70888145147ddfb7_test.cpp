...
int access_rights() {
    ...
    if (::harpia::rbac::decide("", Operation::create, "crew")
...
int rest_api() {
    ...
    int code = 0;
    do {
        auto gl = cli.Get("/api/v1/crew");
        if (!gl || gl.status != 401) { code = 93; break; }
        auto gi = cli.Get("/api/v1/crew/1");
        if (!gi || gi.status != 401) { code = 94; break; }
        auto po = cli.Post("/api/v1/crew", "{}", "application/json");
        if (!po || po.status != 401) { code = 95; break; }
        auto de = cli.Delete("/api/v1/crew/1");
...
int soap_api() {
    ...
    const std::string setEnv = "<soap:Envelope><soap:Body><set><crew></crew></set></soap:Body></soap:Envelope>";
    int code = 0;
    do {
