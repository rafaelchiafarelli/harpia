...
// True iff the envelope carries the correct credential for alarm_event. The handler
// gates every operation on this; exposed so it can be unit-tested directly.
inline bool authorized_alarm_event(const ::tinyxml2::XMLDocument& doc) {
    ...
    return ::harpia::soap::detail::child_text(cred, "user") == "alarm_event" &&
           ::harpia::soap::detail::child_text(cred, "pswd") == "3ac5d8b36fc7dcfb70888145147ddfb7";
}

inline bool authorized_alarm_event(const std::string& soap_xml) {
    ::tinyxml2::XMLDocument doc;
    if (doc.Parse(soap_xml.c_str()) != ::tinyxml2::XML_SUCCESS) return false;
    return authorized_alarm_event(doc);
}
