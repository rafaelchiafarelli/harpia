...
// factory that knows pump_tick's SocketType/bind-vs-connect/origin-id
...
public final class pump_tick_zmq {
    private pump_tick_zmq() {}

    // Compile-time unique sender number (one-to-*: hash(file)+message),
    // the same derivation as the C++ target's origin_id().
    public static final String ORIGIN_ID = "86739202280142613";
    ...
    public static HarpiaZmq.Sender newPublisher(ZContext ctx, String endpoint) {
        return new HarpiaZmq.Sender(ctx, SocketType.PUB, endpoint, true,
            ORIGIN_ID, pump_tick.getDefaultInstance());
    }
    ...
    public static HarpiaZmq.Sender newPublisher(ZContext ctx, String endpoint,
        ...
        return new HarpiaZmq.Sender(ctx, SocketType.PUB, endpoint, true,
            ORIGIN_ID, pump_tick.getDefaultInstance(), curve);
    }
