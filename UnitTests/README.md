...
|------|----------------|-------|
| `test_golden.py`      | tokens, messages, generated `proto/`, `json/`, `zmq/`, `xml/` wrappers, `db/`, `rest/`, `soap/`, `wsdl/`, `gen_tests/` and the `sidecars/` (sql/modifier/access/pswd) match `UnitTests/golden/` | python only |
...
Snapshots live under `UnitTests/golden/`, keyed by the input's md5 hash
...
git diff UnitTests/golden     # review the drift -- this review is the point
```
