...
6. **Doxygen doc-comments are part of the deliverable, not a follow-up
   ...
   not generic boilerplate — see `initiatives/doxygen-generation/doxygen-generation.md`'s pitfall
...
Track H (`thread-1-data-and-keys/histories/schema-evolution/track-h-schema-evolution.md`) and Track B
(`thread-2-transport-and-access/histories/zmq-lifecycle/track-b-zmq-lifecycle.md`) before starting
either. One smaller
...
`initiatives/doxygen-generation/doxygen-generation.md` lives on as a pitfall-table reference, see
...
`initiatives/multi-language-targets/` (`plans/java-target.md` and
`plans/multi-language-targets.md`, the two files this used to cite, were
already deleted before this rename — merged into that standalone plan).
See `multi-language-targets/thread-1-java-target/histories/track-j-java-target.md`'s
own compliance-aware-layer note — there's no separate thin-pointer file
under this plan for it.
...
different, simpler mechanism: every generated file is written via
`Util.util.write_if_different`/`copy_if_different`, which compare full
file content and write through an atomic same-directory temp-file +
`os.replace` — no registry, no `.sha256` sidecars, no start/finish
markers (see `util/CLAUDE.md`).
...
`thread-4-platform-infra/histories/versioning/track-l-versioning.md`, not resolved.
