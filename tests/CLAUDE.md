...
- `test_java_gradle_wiring.py` — session J.2 (`initiatives/multi-language-targets/thread-1-java-target`): `HARPIA_GEN_LANG=java` (default `cpp`, unaffected) stands up a self-contained Gradle project under `<dest>/java/` (`GradleAdapter`, see its `CLAUDE.md`). Structural (pure Python, always run): default/`cpp` don't create `java/` at all; `java` wires `build.gradle`/`settings.gradle` + a copy of every plain message `.proto` (no `_service.proto`, no framework protos — no gRPC yet, that's J.3) carrying J.1's `java_multiple_files`/`java_package` options; write-if-different (stable mtime on an unchanged rerun). Integration (gradle+JDK-gated, not part of the harpia Docker image yet): a real `gradle build` compiles the protobuf-gradle-plugin-generated message classes, then a smoke program constructs one via its generated builder and round-trips a field.
