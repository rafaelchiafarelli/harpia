...
| `test_stage8_db.py`   | Stage 8: generated SQL schema executes in SQLite; CRUDL DAO round-trip (insert→read→update→list→delete); singular FK round-trip (top_users.myUsers→vip_users persisted/loaded via the child DAO); map<K,V> child-table round-trip (data.val.a/b/c); DB↔JSON/XML bulk export/import round-trip | cc + g++ (vendored sqlite); CRUDL/IO parts also protoc |
