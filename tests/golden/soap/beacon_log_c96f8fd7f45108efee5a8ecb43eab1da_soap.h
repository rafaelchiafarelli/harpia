...
#ifndef HARPIA_SOAP_BEACON_LOG_c96f8fd7f45108efee5a8ecb43eab1da
#define HARPIA_SOAP_BEACON_LOG_c96f8fd7f45108efee5a8ecb43eab1da
...
#include "xml/beacon_log_c96f8fd7f45108efee5a8ecb43eab1da_xml.h"   // brings tinyxml2 + the XML runtime

// Minimal SOAP-over-HTTP access for beacon_log, backed by the CRUDL DAO. Register on
// a crow::SimpleApp with a base path; POST a SOAP envelope to <base>/beacon_log:
...
inline std::string envelope_beacon_log(const std::string& body) {
    return "<?xml version=\"1.0\"?>"
           "<soap:Envelope xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\">"
           "<soap:Body>" + body + "</soap:Body></soap:Envelope>";
}

// True iff the envelope carries the correct credential for beacon_log. The handler
// gates every operation on this; exposed so it can be unit-tested directly.
inline bool authorized_beacon_log(const ::tinyxml2::XMLDocument& doc) {
    ...
    return ::harpia::soap::detail::child_text(cred, "user") == "beacon_log" &&
           ::harpia::soap::detail::child_text(cred, "pswd") == "c96f8fd7f45108efee5a8ecb43eab1da";
}

inline bool authorized_beacon_log(const std::string& soap_xml) {
    ::tinyxml2::XMLDocument doc;
    if (doc.Parse(soap_xml.c_str()) != ::tinyxml2::XML_SUCCESS) return false;
    return authorized_beacon_log(doc);
}

inline void register_beacon_log_soap(crow::SimpleApp& app, ::soci::session& db,
    ...
    ::soci::session* dbp = &db;
    app.route_dynamic(base + "/beacon_log").methods(crow::HTTPMethod::POST)(
        ...
        if (!authorized_beacon_log(doc)) {
            res.code = 401;
            reply(envelope_beacon_log(
        ...
        if (name == "get") {
            ...
            const auto* m = op->FirstChildElement();          // the beacon_log element
            ::beacon_log msg;
...
#endif  // HARPIA_SOAP_BEACON_LOG_c96f8fd7f45108efee5a8ecb43eab1da
