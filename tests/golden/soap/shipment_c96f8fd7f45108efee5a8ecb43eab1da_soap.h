...
#ifndef HARPIA_SOAP_SHIPMENT_c96f8fd7f45108efee5a8ecb43eab1da
#define HARPIA_SOAP_SHIPMENT_c96f8fd7f45108efee5a8ecb43eab1da
...
#include "xml/shipment_c96f8fd7f45108efee5a8ecb43eab1da_xml.h"   // brings tinyxml2 + the XML runtime

// Minimal SOAP-over-HTTP access for shipment, backed by the CRUDL DAO. Register on
// a crow::SimpleApp with a base path; POST a SOAP envelope to <base>/shipment:
...
inline std::string envelope_shipment(const std::string& body) {
    return "<?xml version=\"1.0\"?>"
           "<soap:Envelope xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\">"
           "<soap:Body>" + body + "</soap:Body></soap:Envelope>";
}

// True iff the envelope carries the correct credential for shipment. The handler
// gates every operation on this; exposed so it can be unit-tested directly.
inline bool authorized_shipment(const ::tinyxml2::XMLDocument& doc) {
    ...
    return ::harpia::soap::detail::child_text(cred, "user") == "shipment" &&
           ::harpia::soap::detail::child_text(cred, "pswd") == "c96f8fd7f45108efee5a8ecb43eab1da";
}

inline bool authorized_shipment(const std::string& soap_xml) {
    ::tinyxml2::XMLDocument doc;
    if (doc.Parse(soap_xml.c_str()) != ::tinyxml2::XML_SUCCESS) return false;
    return authorized_shipment(doc);
}

inline void register_shipment_soap(crow::SimpleApp& app, ::soci::session& db,
    ...
    ::soci::session* dbp = &db;
    app.route_dynamic(base + "/shipment").methods(crow::HTTPMethod::POST)(
        ...
        if (!authorized_shipment(doc)) {
            res.code = 401;
            reply(envelope_shipment(
        ...
        if (name == "get") {
            ...
            const auto* m = op->FirstChildElement();          // the shipment element
            ::shipment msg;
...
#endif  // HARPIA_SOAP_SHIPMENT_c96f8fd7f45108efee5a8ecb43eab1da
