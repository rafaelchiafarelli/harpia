...
#ifndef HARPIA_SOAP_USERS_c96f8fd7f45108efee5a8ecb43eab1da
#define HARPIA_SOAP_USERS_c96f8fd7f45108efee5a8ecb43eab1da
...
#include "xml/users_c96f8fd7f45108efee5a8ecb43eab1da_xml.h"   // brings tinyxml2 + the XML runtime

// Minimal SOAP-over-HTTP access for users, backed by the CRUDL DAO. Register on
// a crow::SimpleApp with a base path; POST a SOAP envelope to <base>/users:
...
inline std::string envelope_users(const std::string& body) {
    return "<?xml version=\"1.0\"?>"
           "<soap:Envelope xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\">"
           "<soap:Body>" + body + "</soap:Body></soap:Envelope>";
}

// True iff the envelope carries the correct credential for users. The handler
// gates every operation on this; exposed so it can be unit-tested directly.
inline bool authorized_users(const ::tinyxml2::XMLDocument& doc) {
    ...
    return ::harpia::soap::detail::child_text(cred, "user") == "users" &&
           ::harpia::soap::detail::child_text(cred, "pswd") == "c96f8fd7f45108efee5a8ecb43eab1da";
}

inline bool authorized_users(const std::string& soap_xml) {
    ::tinyxml2::XMLDocument doc;
    if (doc.Parse(soap_xml.c_str()) != ::tinyxml2::XML_SUCCESS) return false;
    return authorized_users(doc);
}

inline void register_users_soap(crow::SimpleApp& app, ::soci::session& db,
    ...
    ::soci::session* dbp = &db;
    app.route_dynamic(base + "/users").methods(crow::HTTPMethod::POST)(
        ...
        if (!authorized_users(doc)) {
            res.code = 401;
            reply(envelope_users(
        ...
        if (name == "get") {
            ...
            const auto* m = op->FirstChildElement();          // the users element
            ::users msg;
...
#endif  // HARPIA_SOAP_USERS_c96f8fd7f45108efee5a8ecb43eab1da
