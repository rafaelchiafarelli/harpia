// Generated by harpia Stage 14 (unit tests). Do not edit.
//
// Unit tests for the generated data message, CRUDL DAO and SOAP access:
//   14.1 simple access     -- setter/getter round-trips for every scalar field
//   14.2 database          -- create/read/update/list/remove over in-memory SQLite
//   14.3 access rights      -- the SOAP credential gate accepts the right password
//                             and rejects a wrong/absent one
//   14.4 access modifiers   -- schema constraints from the modifiers are enforced
//                             (PRIMARY KEY uniqueness rejects a duplicate row)
//   14.5 json parser        -- to_json/from_json round-trips the message and
//                             is_valid_json accepts good JSON, rejects garbage
//   14.6 xml parser         -- to_xml/from_xml round-trips the message and
//                             from_xml rejects garbage
//   14.7/14.10 REST API      -- the REST bindings serve real HTTP JSON CRUD
//                             (POST/GET/list/PUT/DELETE/404) over a live server
//   14.8/14.9 SOAP API       -- the SOAP endpoint serves real SOAP-over-HTTP
//                             (set/get with the credential; wrong one -> 401)
//
// Each check returns a distinct non-zero code so a CTest failure points at the
// exact assertion; main() returns 0 only when every check passes.
#include "db/data_734126ee6efdfbd64a1678bf49ee9683_crudl.h"
#include "soap/data_734126ee6efdfbd64a1678bf49ee9683_soap.h"
#include "json/data_734126ee6efdfbd64a1678bf49ee9683_json.h"
#include "xml/data_734126ee6efdfbd64a1678bf49ee9683_xml.h"
#include "rest/data_734126ee6efdfbd64a1678bf49ee9683_rest.h"

#include <cstdint>
#include <string>
#include <thread>
#include <vector>

namespace {

int simple_access() {
    ::data m;
    m.set_id_734126ee6efdfbd64a1678bf49ee9683(1);
    m.set_i(1);
    m.set_j(1);
    m.set_car(static_cast<::grower>(1));
    m.set_status_734126ee6efdfbd64a1678bf49ee9683("status_734126ee6efdfbd64a1678bf49ee9683_a");
    m.set_error_734126ee6efdfbd64a1678bf49ee9683("error_734126ee6efdfbd64a1678bf49ee9683_a");
    m.set_originator_734126ee6efdfbd64a1678bf49ee9683("originator_734126ee6efdfbd64a1678bf49ee9683_a");
    if (m.id_734126ee6efdfbd64a1678bf49ee9683() != 1) return 10;
    if (m.i() != 1) return 11;
    if (m.j() != 1) return 12;
    if (m.car() != static_cast<::grower>(1)) return 13;
    if (m.status_734126ee6efdfbd64a1678bf49ee9683() != "status_734126ee6efdfbd64a1678bf49ee9683_a") return 14;
    if (m.error_734126ee6efdfbd64a1678bf49ee9683() != "error_734126ee6efdfbd64a1678bf49ee9683_a") return 15;
    if (m.originator_734126ee6efdfbd64a1678bf49ee9683() != "originator_734126ee6efdfbd64a1678bf49ee9683_a") return 16;
    return 0;
}

int database_roundtrip() {
    ::sqlite3* db = nullptr;
    if (::sqlite3_open(":memory:", &db) != SQLITE_OK) return 20;
    harpia::db::data_dao dao(db);
    if (!dao.create_table()) return 21;

    // create + read back row 1
    ::data a;
    a.set_id_734126ee6efdfbd64a1678bf49ee9683(1);
    a.set_i(1);
    a.set_j(1);
    a.set_car(static_cast<::grower>(1));
    a.set_status_734126ee6efdfbd64a1678bf49ee9683("status_734126ee6efdfbd64a1678bf49ee9683_a");
    a.set_error_734126ee6efdfbd64a1678bf49ee9683("error_734126ee6efdfbd64a1678bf49ee9683_a");
    a.set_originator_734126ee6efdfbd64a1678bf49ee9683("originator_734126ee6efdfbd64a1678bf49ee9683_a");
    if (!dao.create(a)) return 22;
    ::data got;
    if (!dao.read(1, &got)) return 23;
    if (got.i() != 1) return 24;
    if (got.j() != 1) return 24;
    if (got.car() != static_cast<::grower>(1)) return 24;
    if (got.status_734126ee6efdfbd64a1678bf49ee9683() != "status_734126ee6efdfbd64a1678bf49ee9683_a") return 24;
    if (got.error_734126ee6efdfbd64a1678bf49ee9683() != "error_734126ee6efdfbd64a1678bf49ee9683_a") return 24;
    if (got.originator_734126ee6efdfbd64a1678bf49ee9683() != "originator_734126ee6efdfbd64a1678bf49ee9683_a") return 24;

    // update an existing row
    ::data upd = a;
    upd.set_status_734126ee6efdfbd64a1678bf49ee9683("status_734126ee6efdfbd64a1678bf49ee9683_u");
    if (!dao.update(upd)) return 25;
    ::data got2;
    if (!dao.read(1, &got2)) return 26;
    if (got2.status_734126ee6efdfbd64a1678bf49ee9683() != "status_734126ee6efdfbd64a1678bf49ee9683_u") return 27;

    // a second row, then list + delete
    ::data b;
    b.set_id_734126ee6efdfbd64a1678bf49ee9683(2);
    b.set_i(1);
    b.set_j(1);
    b.set_car(static_cast<::grower>(1));
    b.set_status_734126ee6efdfbd64a1678bf49ee9683("status_734126ee6efdfbd64a1678bf49ee9683_b");
    b.set_error_734126ee6efdfbd64a1678bf49ee9683("error_734126ee6efdfbd64a1678bf49ee9683_b");
    b.set_originator_734126ee6efdfbd64a1678bf49ee9683("originator_734126ee6efdfbd64a1678bf49ee9683_b");
    if (!dao.create(b)) return 28;
    std::vector<::data> all;
    if (!dao.list(&all) || all.size() != 2) return 29;
    if (!dao.remove(1)) return 30;
    ::data gone;
    if (dao.read(1, &gone)) return 31;

    ::sqlite3_close(db);
    return 0;
}

int access_rights() {
    const std::string good = "<soap:Envelope><soap:Header><credentials><user>data</user><pswd>734126ee6efdfbd64a1678bf49ee9683</pswd></credentials></soap:Header><soap:Body/></soap:Envelope>";
    const std::string wrong = "<soap:Envelope><soap:Header><credentials><user>data</user><pswd>wrong-password</pswd></credentials></soap:Header><soap:Body/></soap:Envelope>";
    const std::string none = "<soap:Envelope><soap:Body/></soap:Envelope>";
    if (!harpia::soap::authorized_data(good)) return 50;
    if (harpia::soap::authorized_data(wrong)) return 51;
    if (harpia::soap::authorized_data(none)) return 52;
    return 0;
}

int access_modifiers() {
    ::sqlite3* db = nullptr;
    if (::sqlite3_open(":memory:", &db) != SQLITE_OK) return 40;
    harpia::db::data_dao dao(db);
    if (!dao.create_table()) return 41;
    ::data a;
    a.set_id_734126ee6efdfbd64a1678bf49ee9683(1);
    a.set_i(1);
    a.set_j(1);
    a.set_car(static_cast<::grower>(1));
    a.set_status_734126ee6efdfbd64a1678bf49ee9683("status_734126ee6efdfbd64a1678bf49ee9683_a");
    a.set_error_734126ee6efdfbd64a1678bf49ee9683("error_734126ee6efdfbd64a1678bf49ee9683_a");
    a.set_originator_734126ee6efdfbd64a1678bf49ee9683("originator_734126ee6efdfbd64a1678bf49ee9683_a");
    if (!dao.create(a)) return 42;
    // PRIMARY KEY is unique: a duplicate key must be rejected
    ::data dup;
    dup.set_id_734126ee6efdfbd64a1678bf49ee9683(1);
    dup.set_i(1);
    dup.set_j(1);
    dup.set_car(static_cast<::grower>(1));
    dup.set_status_734126ee6efdfbd64a1678bf49ee9683("status_734126ee6efdfbd64a1678bf49ee9683_b");
    dup.set_error_734126ee6efdfbd64a1678bf49ee9683("error_734126ee6efdfbd64a1678bf49ee9683_b");
    dup.set_originator_734126ee6efdfbd64a1678bf49ee9683("originator_734126ee6efdfbd64a1678bf49ee9683_b");
    if (dao.create(dup)) return 43;
    ::sqlite3_close(db);
    return 0;
}

int json_parser() {
    ::data a;
    a.set_id_734126ee6efdfbd64a1678bf49ee9683(1);
    a.set_i(1);
    a.set_j(1);
    a.set_car(static_cast<::grower>(1));
    a.set_status_734126ee6efdfbd64a1678bf49ee9683("status_734126ee6efdfbd64a1678bf49ee9683_a");
    a.set_error_734126ee6efdfbd64a1678bf49ee9683("error_734126ee6efdfbd64a1678bf49ee9683_a");
    a.set_originator_734126ee6efdfbd64a1678bf49ee9683("originator_734126ee6efdfbd64a1678bf49ee9683_a");
    std::string js;
    if (!harpia::json::to_json(a, &js)) return 60;
    ::data b;
    if (!harpia::json::from_json(js, &b)) return 61;
    if (b.id_734126ee6efdfbd64a1678bf49ee9683() != 1) return 62;
    if (b.i() != 1) return 62;
    if (b.j() != 1) return 62;
    if (b.car() != static_cast<::grower>(1)) return 62;
    if (b.status_734126ee6efdfbd64a1678bf49ee9683() != "status_734126ee6efdfbd64a1678bf49ee9683_a") return 62;
    if (b.error_734126ee6efdfbd64a1678bf49ee9683() != "error_734126ee6efdfbd64a1678bf49ee9683_a") return 62;
    if (b.originator_734126ee6efdfbd64a1678bf49ee9683() != "originator_734126ee6efdfbd64a1678bf49ee9683_a") return 62;
    if (!harpia::json::is_valid_json(js)) return 63;
    if (harpia::json::is_valid_json("this is not json")) return 64;
    return 0;
}

int xml_parser() {
    ::data a;
    a.set_id_734126ee6efdfbd64a1678bf49ee9683(1);
    a.set_i(1);
    a.set_j(1);
    a.set_car(static_cast<::grower>(1));
    a.set_status_734126ee6efdfbd64a1678bf49ee9683("status_734126ee6efdfbd64a1678bf49ee9683_a");
    a.set_error_734126ee6efdfbd64a1678bf49ee9683("error_734126ee6efdfbd64a1678bf49ee9683_a");
    a.set_originator_734126ee6efdfbd64a1678bf49ee9683("originator_734126ee6efdfbd64a1678bf49ee9683_a");
    const std::string xs = ::harpia::xml::to_xml(a);
    if (xs.empty()) return 70;
    ::data b;
    if (!::harpia::xml::from_xml(xs, &b)) return 71;
    if (b.id_734126ee6efdfbd64a1678bf49ee9683() != 1) return 72;
    if (b.i() != 1) return 72;
    if (b.j() != 1) return 72;
    if (b.car() != static_cast<::grower>(1)) return 72;
    if (b.status_734126ee6efdfbd64a1678bf49ee9683() != "status_734126ee6efdfbd64a1678bf49ee9683_a") return 72;
    if (b.error_734126ee6efdfbd64a1678bf49ee9683() != "error_734126ee6efdfbd64a1678bf49ee9683_a") return 72;
    if (b.originator_734126ee6efdfbd64a1678bf49ee9683() != "originator_734126ee6efdfbd64a1678bf49ee9683_a") return 72;
    ::data c;
    if (::harpia::xml::from_xml("not xml at all", &c)) return 73;
    return 0;
}

int rest_api() {
    ::sqlite3* db = nullptr;
    if (::sqlite3_open(":memory:", &db) != SQLITE_OK) return 80;
    harpia::db::data_dao dao(db);
    if (!dao.create_table()) return 81;
    ::httplib::Server svr;
    harpia::rest::register_data(svr, db, "/api/v1");
    const int port = svr.bind_to_any_port("127.0.0.1");
    if (port <= 0) return 82;
    std::thread t([&]{ svr.listen_after_bind(); });
    svr.wait_until_ready();
    ::httplib::Client cli("127.0.0.1", port);
    int code = 0;
    do {
        ::data a;
        a.set_id_734126ee6efdfbd64a1678bf49ee9683(1);
        a.set_i(1);
        a.set_j(1);
        a.set_car(static_cast<::grower>(1));
        a.set_status_734126ee6efdfbd64a1678bf49ee9683("status_734126ee6efdfbd64a1678bf49ee9683_a");
        a.set_error_734126ee6efdfbd64a1678bf49ee9683("error_734126ee6efdfbd64a1678bf49ee9683_a");
        a.set_originator_734126ee6efdfbd64a1678bf49ee9683("originator_734126ee6efdfbd64a1678bf49ee9683_a");
        std::string body;
        if (!::harpia::json::to_json(a, &body)) { code = 83; break; }
        auto post = cli.Post("/api/v1/data", body, "application/json");
        if (!post || post->status != 201) { code = 84; break; }
        auto got = cli.Get("/api/v1/data/1");
        if (!got || got->status != 200) { code = 85; break; }
        if (got->body.find("status_734126ee6efdfbd64a1678bf49ee9683_a") == std::string::npos) { code = 86; break; }
        auto lst = cli.Get("/api/v1/data");
        if (!lst || lst->status != 200) { code = 87; break; }
        ::data b = a;
        b.set_status_734126ee6efdfbd64a1678bf49ee9683("status_734126ee6efdfbd64a1678bf49ee9683_u");
        std::string bb;
        if (!::harpia::json::to_json(b, &bb)) { code = 88; break; }
        auto put = cli.Put("/api/v1/data/1", bb, "application/json");
        if (!put || put->status != 204) { code = 89; break; }
        auto g2 = cli.Get("/api/v1/data/1");
        if (!g2 || g2->body.find("status_734126ee6efdfbd64a1678bf49ee9683_u") == std::string::npos) { code = 90; break; }
        auto del = cli.Delete("/api/v1/data/1");
        if (!del || del->status != 204) { code = 91; break; }
        auto gone = cli.Get("/api/v1/data/1");
        if (!gone || gone->status != 404) { code = 92; break; }
    } while (false);
    svr.stop(); t.join(); ::sqlite3_close(db);
    return code;
}

int soap_api() {
    ::sqlite3* db = nullptr;
    if (::sqlite3_open(":memory:", &db) != SQLITE_OK) return 100;
    harpia::db::data_dao dao(db);
    if (!dao.create_table()) return 101;
    ::httplib::Server svr;
    harpia::soap::register_data_soap(svr, db, "/soap");
    const int port = svr.bind_to_any_port("127.0.0.1");
    if (port <= 0) return 102;
    std::thread t([&]{ svr.listen_after_bind(); });
    svr.wait_until_ready();
    ::httplib::Client cli("127.0.0.1", port);
    const std::string hdr = "<soap:Header><credentials><user>data</user><pswd>734126ee6efdfbd64a1678bf49ee9683</pswd></credentials></soap:Header>";
    const std::string badhdr = "<soap:Header><credentials><user>data</user><pswd>nope</pswd></credentials></soap:Header>";
    int code = 0;
    do {
        ::data a;
        a.set_id_734126ee6efdfbd64a1678bf49ee9683(1);
        a.set_i(1);
        a.set_j(1);
        a.set_car(static_cast<::grower>(1));
        a.set_status_734126ee6efdfbd64a1678bf49ee9683("status_734126ee6efdfbd64a1678bf49ee9683_a");
        a.set_error_734126ee6efdfbd64a1678bf49ee9683("error_734126ee6efdfbd64a1678bf49ee9683_a");
        a.set_originator_734126ee6efdfbd64a1678bf49ee9683("originator_734126ee6efdfbd64a1678bf49ee9683_a");
        const std::string mx = ::harpia::xml::to_xml(a);
        const std::string setEnv = "<soap:Envelope>" + hdr + "<soap:Body><set>" + mx + "</set></soap:Body></soap:Envelope>";
        auto s = cli.Post("/soap/data", setEnv, "text/xml");
        if (!s || s->status != 200 || s->body.find("<ok>true</ok>") == std::string::npos) { code = 103; break; }
        const std::string badEnv = "<soap:Envelope>" + badhdr + "<soap:Body><get><id>1</id></get></soap:Body></soap:Envelope>";
        auto na = cli.Post("/soap/data", badEnv, "text/xml");
        if (!na || na->status != 401) { code = 104; break; }
        const std::string getEnv = "<soap:Envelope>" + hdr + "<soap:Body><get><id>1</id></get></soap:Body></soap:Envelope>";
        auto g = cli.Post("/soap/data", getEnv, "text/xml");
        if (!g || g->status != 200 || g->body.find("getResponse") == std::string::npos) { code = 105; break; }
        if (g->body.find("status_734126ee6efdfbd64a1678bf49ee9683_a") == std::string::npos) { code = 106; break; }
    } while (false);
    svr.stop(); t.join(); ::sqlite3_close(db);
    return code;
}

}  // namespace

int main() {
    if (int rc = simple_access()) return rc;
    if (int rc = database_roundtrip()) return rc;
    if (int rc = access_rights()) return rc;
    if (int rc = access_modifiers()) return rc;
    if (int rc = json_parser()) return rc;
    if (int rc = xml_parser()) return rc;
    if (int rc = rest_api()) return rc;
    if (int rc = soap_api()) return rc;
    return 0;
}
