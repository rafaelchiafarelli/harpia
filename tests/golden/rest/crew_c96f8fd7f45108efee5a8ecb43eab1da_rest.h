...
#ifndef HARPIA_REST_CREW_c96f8fd7f45108efee5a8ecb43eab1da
#define HARPIA_REST_CREW_c96f8fd7f45108efee5a8ecb43eab1da
...
// crew declares a "pagination[size]" field (its size becomes the default
...
// the request must carry X-User: crew and X-Pswd: c96f8fd7f45108efee5a8ecb43eab1da, or it is rejected
// with HTTP 401 before the operation runs.
namespace harpia {
namespace rest {

// True iff the request carries the correct credential for crew. Exposed so it
// can be unit-tested directly.
inline bool authorized_crew(const crow::request& req) {
    return req.get_header_value("X-User") == "crew" &&
           req.get_header_value("X-Pswd") == "c96f8fd7f45108efee5a8ecb43eab1da";
}

// content negotiation: XML when asked for, JSON otherwise
inline bool wants_xml_crew(const crow::request& req) {
    return req.get_header_value("Accept").find("xml") != std::string::npos;
}
inline bool body_xml_crew(const crow::request& req) {
    return req.get_header_value("Content-Type").find("xml") != std::string::npos;
}
inline bool parse_crew(const crow::request& req, ::crew* msg) {
    return body_xml_crew(req) ? ::harpia::xml::from_xml(req.body, msg)
                                : ::harpia::json::from_json(req.body, msg);
}
inline void serialize_crew(const crow::request& req,
                             const ::crew& msg, crow::response& res) {
    if (wants_xml_crew(req)) {
...
inline void register_crew(crow::SimpleApp& app, ::soci::session& db,
                            const std::string& base) {
    const std::string col = base + "/crew";
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::GET)(
        [dbp](const crow::request& req, crow::response& res) {
            if (!authorized_crew(req)) { res.code = 401; res.end(); return; }
    ...
    app.route_dynamic(col).methods(crow::HTTPMethod::POST)(
        [dbp](const crow::request& req, crow::response& res) {
            ...
            ::harpia::db::crew_dao dao(*dbp);
            ::crew msg;
            if (!parse_crew(req, &msg)) { res.code = 400; res.end(); return; }
...
#endif  // HARPIA_REST_CREW_c96f8fd7f45108efee5a8ecb43eab1da
