...
it must be able to be "continued". This means that, if it stop or failed for any reason, it must be able to be continued.
  **Superseded by `plans/crash-interrupt-recovery.md`.** The original idea
  ...
  generate is just "run it again" -- content-mismatch is its own staleness
  marker, and atomic replace means no output is ever observed half-written.
  No registry, no `.sha256` sidecars, no start/finish markers.

  Original spec text, for history:
