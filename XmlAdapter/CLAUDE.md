...
- Runtime quirk (important): a singular field is emitted only when `HasField` is true whenever the field has REAL presence (`FieldDescriptor::has_presence()`) — always true for **message** fields (otherwise an absent child would round-trip as an empty *present* child, e.g. making an FK adapter persist a phantom row), and true for a **scalar** field only when the `.harpia` schema marks it `optional` (proto3 explicit presence — see `protoFile/CLAUDE.md` and `plans/message-versioning.md` §4). An ordinary (non-`optional`) proto3 scalar has no presence to track and is always emitted with its default.
