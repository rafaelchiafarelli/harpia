...
## Key facts / gotchas
- Unlike JSON, protobuf has NO built-in XML → the runtime does reflection walking rather than per-field generated code. **`YamlAdapter/` (the serialization epic) is a close copy of this module** — same reflection-walk approach, same `FieldDescriptor::name()` direct-init and `#undef GetMessage` gotchas — for the same reason (no built-in YAML either). serialization task 2 (`SerializeAdapter/`) put a single `harpia::serialize::to_string(msg, Format)` façade over all three — it dispatches to `harpia::xml::to_xml` here unchanged.
